...
`run` passes a second argument with `currentPath` — the pathname that
triggered this execution. Use it to show different sidebar content on
different pages without re-declaring the sidebar:
...
`currentPath` reflects the URL that triggered the current run —
navigation fires a new run, so `currentPath` is always up to date.
