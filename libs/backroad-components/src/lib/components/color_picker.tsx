...
export const ColorPicker: BackroadComponentRenderer<'color_picker'> = (
  props
) => {
  ...
  return (
    <div className="form-control w-full max-w-xs relative">
      <label className="label">
        <span className="backroad-label">
          {props.args.label || 'Pick a color'}
        </span>
      </label>
      <div
      ...
      ></div>
      {isOpen && (
        <div ref={popover} className="absolute top-[80px]">
          <HexColorPicker
            color={value}
            onChange={(newColor) => {
              setValue(newColor);
